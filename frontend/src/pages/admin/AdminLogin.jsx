...
function AdminLogin() {
  ...
  // Check for OAuth success or error messages in URL params
  useEffect(() => {
  ...
  const handleOAuthSuccess = async () => {
    try {
      ...
      // Refresh the authentication context to get user data
      await refreshUser();
  ...
  const getErrorMessage = (error) => {
    const errorMessages = {
      ...
      unauthorized_domain: "Please use your myIIT email account (@g.msuiit.edu.ph).",
            ...
            <div className="w-full max-w-sm mx-auto space-y-8">
              ...
              <div className="flex flex-col items-center">
                <button
                  ...
                  disabled={isLoading}
                  className="w-full flex items-center justify-center gap-3 px-6 py-4 bg-white border-2 border-gray-300 rounded-xl hover:bg-gray-50 hover:border-red-600 transition-all duration-200 shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {isLoading ? (
                    <>
